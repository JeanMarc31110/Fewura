...
<#
.SYNOPSIS
Fewura Professional Windows Installer Deployment Script
Downloads and silently installs the latest Fewura release.

.PARAMETER Repo
GitHub repository in format owner/repo (default: JeanMarc31110/Fewura)
...
.\install-client-pro.ps1 -Repo "JeanMarc31110/Fewura"
.\install-client-pro.ps1 -Token $env:GITHUB_TOKEN
#>

param(
    [string]$Repo = "JeanMarc31110/Fewura",
    [string]$Token = ""
)
...
Write-Host "Fewura Professional Windows Installer" -ForegroundColor Green
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
function Install-Silently {
    ...
    try {
        ...
        Write-Host "Installing Fewura silently..."
...
try {
    ...
    $setupExe = $release.assets | Where-Object { $_.name -match "Fewura_Setup.*\.exe$" } | Select-Object -First 1
    ...
    # Download
    $tempDir = Join-Path $env:TEMP "Fewura-Installer"
    ...
    Write-Host ""
    Write-Host "✓ Fewura installation completed successfully!" -ForegroundColor Green
    Write-Host ""
    exit 0
