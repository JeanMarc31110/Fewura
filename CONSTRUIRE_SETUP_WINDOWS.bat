...
if not exist "dist\FEWURA_Prospect\FEWURA_Prospect.exe" goto :error
...
if not defined ISCC if defined ChocolateyInstall for /r "%ChocolateyInstall%" %%I in (ISCC.exe) do if not defined ISCC set "ISCC=%%I"
if not defined ISCC for /r "%ProgramFiles%" %%I in (ISCC.exe) do if not defined ISCC set "ISCC=%%I"
if not defined ISCC if defined ProgramFiles(x86) for /r "%ProgramFiles(x86)%" %%I in (ISCC.exe) do if not defined ISCC set "ISCC=%%I"
if not defined ISCC if exist "%LOCALAPPDATA%" for /r "%LOCALAPPDATA%" %%I in (ISCC.exe) do if not defined ISCC set "ISCC=%%I"
